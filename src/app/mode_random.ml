...
let run players priorities ~help_alchemists splits () =
  ...
  let points_spread = Int.Table.create () in
  Map.iter splits ~f:(fun (_split, random) -> Hashtbl.incr points_spread random.total_points);
  print_endline
    (sprintf
       !"Splits imbalance <=%d: %d\nCombinations points <=%d: %d\nBreakdown: %{sexp: (int * int) list}"
       max_imbalance (List.length acceptable_splits) max_points num_splits
